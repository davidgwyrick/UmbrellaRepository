...
filepath=['J:\TannerBertGroup\Sims\David_Wyrick\UmbrellaRepository\DataFiles\CaTransient\Scaled_LandTwitchHuman_pCapeak=5.6\TimeSeriesAvg_pCa_5.60.txt'];
Data = importdata(filepath);
...
filepath=['J:\TannerBertGroup\Sims\David_Wyrick\UmbrellaRepository\DataFiles\CaTransient\Scaled_LandTwitchMouse_pCapeak=5.6\TimeSeriesAvg_pCa_5.60.txt'];
Data = importdata(filepath);
TS_Force(8) = {Data.data(:,2)};
TS_Ca(8) = {Data.data(:,12)};
Ca2state(8) = {Data.data(:,10)};

filepath=['J:\TannerBertGroup\Sims\David_Wyrick\UmbrellaRepository\DataFiles\CaTransient\Scaled_LandTwitchRat_pCapeak=5.6\TimeSeriesAvg_pCa_5.60.txt'];
Data = importdata(filepath);
...
subplot(2,2,1)
hold on;
plot(0:500,fmouse(end-500:end)/fmMax,'-','Color',[0.8 0.2 0.2],'LineWidth',2),hold on
plot(timeTrace6,avgTrace8Y/ym8,'-c','LineWidth',1.5),hold on
plot(timeTrace6,filtfilt(f5,1,TF3state8),'--c','LineWidth',1.5),hold on
plot(timeTrace6,avgTrace5Y/ym5,'-b','LineWidth',1.5),hold on
plot(timeTrace6,filtfilt(f5,1,TF3state5),'--k','LineWidth',1.5)
% plot(timeTrace2,avgTrace2Y/ym2,'-g','LineWidth',1.5),hold on
xlim([0 500]); 
xlabel('Time (ms)');
ylabel('Normalized Force');
title('Mouse');
legend('Land Model','Tanner Model-Land Mouse Ca Transient','TF3 Fraction','Tanner Model-100ms pCa5.6 Twitch','TF3 fraction');

subplot(2,2,2)
hold on;
plot(0:500,frat(end-500:end)/frMax,'-','Color',[0.2 0.2 0.8],'LineWidth',2),hold on
plot(timeTrace6,avgTrace9Y/ym9,'-m','LineWidth',1.5)
plot(timeTrace6,filtfilt(f5,1,TF3state9),'--m','LineWidth',1.5),hold on
plot(timeTrace6,avgTrace5Y/ym5,'-g','LineWidth',1.5)
plot(timeTrace6,filtfilt(f5,1,TF3state5),'--k','LineWidth',1.5)
% plot(timeTrace2,avgTrace2Y/ym2,'-g','LineWidth',1.5),hold on
xlim([0 500]); 
xlabel('Time (ms)');
ylabel('Normalized Force');
title('Rat');
legend('Land Model','Tanner Model-Land Rat Ca Transient','TF3 Fraction','Tanner Model-100ms pCa5.6 Twitch','TF3 fraction');

subplot(2,2,3)
hold on;
plot(0:1000,fhuman(end-1000:end)/fhMax,'-','Color',[0.2 0.8 0.2],'LineWidth',2),hold on
plot(timeTrace6,avgTrace7Y/ym7,'-r','LineWidth',1.5),hold on
plot(timeTrace6,filtfilt(f5,1,TF3state7),'--r','LineWidth',1.5),hold on
plot(timeTrace6,avgTrace6Y/ym5,'-y','LineWidth',1.5),hold on
plot(timeTrace6,filtfilt(f5,1,TF3state6),'--k','LineWidth',1.5)
% plot(timeTrace2,avgTrace3Y/ym3,'-b','LineWidth',1.5),hold on
xlim([0 500]); 
xlabel('Time (ms)');
ylabel('Normalized Force');
title('Human');
legend('Land Model','Tanner Model-Land Human Ca Transient','TF3 Fraction','Tanner Model-300ms pCa5.6 Twitch','TF3 fraction');

%legend('Mouse', 'Rat', 'Human','LandHuman','LandMouse','LandRat');
