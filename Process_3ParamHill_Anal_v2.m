...
function [header, vals]=Process_3ParamHill_Anal_v2(xdata, ydata)

% Create the 3-parm Hill function that you will be fitting the data to
% disp('For reference: maxval=x(1); slope=x(2); K50=x(3);')
f=@(x,xdata)(x(1)./(1+10.^(x(2)*(xdata - x(3)))));
...
[beta,resid,J,COVbeta,MSE] = nlinfit(xdata,ydata,f,[maxGuess slopeGuess midGuess]);
...
vals=[];
vals(1:3,1)=beta';
vals(1:3,2)=[sqrt(COVbeta(1,1));sqrt(COVbeta(2,2));sqrt(COVbeta(3,3))];
vals(1:3,3)=[COVbeta(1,1);COVbeta(2,2);COVbeta(3,3)];
vals(1:3,4:5)=ci_cov_05;
vals(1:3,6:7)=ci_cov_01;
