...
Force=zeros(12,3);
Stdv_Force=zeros(12,3);

%% Loop To gather data
index=1;
for ii=1:length(Koff_range)
    Koff=Koff_range(ii);
    for ll=1:length(pCa_Range)
        pCa=pCa_Range(ll);

        foldername=['SS_TFrates_koff=',num2str(Koff),' RuOff=',num2str(RuOff),' CaOff=',num2str(CaOff)];
        filepath=['J:\TannerBertGroup\Sims\David_Wyrick\UmbrellaRepository\DataFiles\TF_RatesForce_pCa\',foldername,'\SSData_pCa_',num2str(pCa,'% 10.2f'),'_ROI_A.txt'];
        if pCa == 4,disp(foldername);end
        disp(pCa)
        Data = importdata(filepath);
        Force(ll,ii)=mean(Data.data(:,3));
        Stdv_Force(ll,ii)=std(Data.data(:,3));
...
label1=['koff=5 RuOff=5 CaOff=100 pCa50=',num2str(coeff1(3,1)),' nh=',num2str(coeff1(2,1))];
errorbar(pCa_Range,Force(:,2),Stdv_Force(:,2),'-og','LineWidth',.5,'MarkerSize',2),hold on
label2=['koff=25 RuOff=5 CaOff=100 pCa50=',num2str(coeff2(3,1)),' nh=',num2str(coeff2(2,1))];
errorbar(pCa_Range,Force(:,3),Stdv_Force(:,3),'-ob','LineWidth',.5,'MarkerSize',2),hold on
label3=['koff=100 RuOff=5 CaOff=100 pCa50=',num2str(coeff3(3,1)),' nh=',num2str(coeff3(2,1))];
